...
func (r *ChecklistPolicyReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	if err := r.reconcileScheduler(ctx, &policy); err != nil {
		...
		// Set degraded condition with specific reason
	...
	// Set condition based on prior scan results
	if policy.Status.Summary != nil && policy.Status.Summary.Fail > 0 {
		msg := fmt.Sprintf("Last scan: %d pass, %d warn, %d fail",
			policy.Status.Summary.Pass, policy.Status.Summary.Warn, policy.Status.Summary.Fail)
		meta.SetStatusCondition(&policy.Status.Conditions, metav1.Condition{
			Type:               "Degraded",
			Status:             metav1.ConditionTrue,
			Reason:             "ChecksFailed",
...
func (r *ChecklistPolicyReconciler) reconcileScheduler(ctx context.Context, policy *dtmv1alpha1.ChecklistPolicy) error {
	...
	if err != nil {
		return fmt.Errorf("LLM secret '%s' not found in namespace '%s': %w", policy.Spec.LLM.SecretRef.Name, policy.Namespace, err)
	}
